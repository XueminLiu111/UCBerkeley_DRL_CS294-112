...
class Agent(object):
    def __init__(self, computation_graph_args, sample_trajectory_args, estimate_advantage_args):
        ...
        self.size_actor = computation_graph_args['size_actor']
        self.size_critc = computation_graph_args['size_critc']
        self.n_layers_actor = computation_graph_args['n_layers_actor']
        self.n_layers_critic = computation_graph_args['n_layers_critic']
        self.learning_rate_actor = computation_graph_args['learning_rate_actor']
    ...
    def policy_forward_pass(self, sy_ob_no):
        ...
        if self.discrete:
            # YOUR_HW2 CODE_HERE
            sy_logits_na = build_mlp(sy_ob_no, self.ac_dim, 'policy_mlp', self.n_layers_actor, self.size_actor)
            return sy_logits_na
        else:
            # YOUR_HW2 CODE_HERE
            sy_mean = build_mlp(sy_ob_no, self.ac_dim, 'policy_mlp', self.n_layers_actor, self.size_actor)
...
def train_AC(
    ...
    computation_graph_args = {
        'n_layers_actor': n_layers_actor,
        'n_layers_critic':n_layers_critic,
        'ob_dim': ob_dim,
        'ac_dim': ac_dim,
        'discrete': discrete,
        'size_actor': size_actor,
        'size_critc': size_critc,
    ...
    num_e = int((seed - 1 )/10 + 1)
    #video_path_init = 'results/' + env_name + '_e' + str(num_e) +'_itr0' + '.mp4'
    video_path_init = 'results/'+ env_name +'/' + env_name + '_' +str(num_target_updates) +'_' + str(num_grad_steps_per_target_update)\
                    + '_e' + str(num_e) +'_b' + str(min_timesteps_per_batch)+ '_lractor' + str(learning_rate_actor) \
                    +  '_lrcritic' + str(learning_rate_critic)+ 'hlactor' + str(n_layers_actor) +'hlcritic' + str(n_layers_critic) +'_itr0' + '.mp4'
    episode_rews = []
    ...
    for itr in range(n_iter):
        ...
        if (itr+1)%20==0:
            ...
            video_path = 'results/' + env_name +'/'+ env_name + '_' +str(num_target_updates) +'_' + str(num_grad_steps_per_target_update)\
                    + '_e' + str(num_e) +'_b' + str(min_timesteps_per_batch) + '_lractor' + str(learning_rate_actor) \
                    +  '_lrcritic' + str(learning_rate_critic) + 'hlactor' + str(n_layers_actor) +'hlcritic' + str(n_layers_critic) + '_itr' +str(itr) + '.mp4'
            episode_rew = record_animation(agent, env, video_path)
            episode_rews.append(episode_rew)
    print(episode_rews)
    '''
    rews_path = 'results/'+ env_name +'/' + env_name +  '_' +str(num_target_updates) +'_' + str(num_grad_steps_per_target_update)\
                + '_e' + str(num_e) +'_b' + str(min_timesteps_per_batch) + '_lractor' + str(learning_rate_actor) \
                + '_lrcritic' + str(learning_rate_critic) + 'hlactor' + str(n_layers_actor) +'hlcritic' + str(n_layers_critic) + '_test_rewards.npy'
    '''
    rews_path = video_path[:-10] + '_test_rewards.npy' 
...
def main():
    ...
    parser.add_argument('--learning_rate_actor', '-lractor', type=float, default=5e-3)
    parser.add_argument('--learning_rate_critic', '-lrcritic', type=float, default=5e-3)
    parser.add_argument('--dont_normalize_advantages', '-dna', action='store_true')
    parser.add_argument('--num_target_updates', '-ntu', type=int, default=10)
    parser.add_argument('--num_grad_steps_per_target_update', '-ngsptu', type=int, default=10)
    parser.add_argument('--seed', type=int, default=1)
    parser.add_argument('--n_experiments', '-e', type=int, default=1)
    parser.add_argument('--n_layers_actor', '-lactor', type=int, default=2)
    parser.add_argument('--n_layers_critic', '-lcritic', type=int, default=2)
    parser.add_argument('--size_actor', '-sactor', type=int, default=64)
    parser.add_argument('--size_critc', '-scritic', type=int, default=64)
    ...
    for e in range(args.n_experiments):
        ...
        def train_func():
            train_AC(
                ...
                seed=seed,
                n_layers_actor=args.n_layers_actor,
                n_layers_critic=args.n_layers_critic,
                size_actor=args.size_actor,
                size_critc=args.size_critc
                )
