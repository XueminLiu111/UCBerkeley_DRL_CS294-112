...
class QLearner(object):
    ...
    def __init__(
        ...
        if not double_q:
            q_t = tf.reduce_max(target_q, axis=1)   #Computes the maximum of elements across dimensions of q_t.
                                                    #select max(target_q)
        else:
            q_temp = q_func(obs_tp1_float, self.num_actions, scope='q_func', reuse=True)
            act_temp = tf.argmax(q_temp, axis=1)    #DoubleDQN select action which max(q_temp) as action in target
    ...
    def step_env(self, update_step,video_recorder):
        ...
        #pdb.set_trace()
        #action_ = act # self.session.run(self.best_act, feed_dict={self.obs_t_ph:[obs]})[0]
        
        #video_recorder = None
        #video_path = 'results/' + 'Pong_itr' + str(update_step) +'.mp4'
        #video_recorder = VideoRecorder(self.env, video_path, enabled=video_path is not None)
        if update_step==0 or (update_step+1)%(40000)==0:
            self.env.render()
            print("Saving video.")
...
def record_animation(agent, env, video_path):
    ...
    env.close()  # close the unfinished episode to start a new env.reset()
    video_recorder = None
    video_recorder = VideoRecorder(
        env, video_path, enabled=video_path is not None)

    obs_0, done = env.reset(), False
    episode_rew = 0
    step = 0
    obs = obs_0
    while not done:
    #while step<=max_step and not done:
        env.render()
        video_recorder.capture_frame()
        if obs.all() == obs_0.all():
            obs = np.concatenate((obs_0, obs_0, obs_0, obs_0), axis=2, out=None)
            #pdb.set_trace()
       
        #action = agent.env.action_space.sample()  # for test

        #obs = agent.replay_buffer.encode_recent_observation()
        pdb.set_trace()
        agent.best_act = tf.local_variables_initializer()  # how to use the pretrained model??
        action = agent.session.run(agent.best_act, feed_dict={agent.obs_t_ph:[obs]})[0]
        #pdb.set_trace()
...
def learn(*args, **kwargs):
    ...
    update_step = 0
    #pdb.set_trace()
    env_name = alg.env.unwrapped.spec.id
    #episode_rews = []

    
    while not alg.stopping_criterion_met():
    #while update_step <= 10000:   

        if (update_step+1)%10000 == 0:
            print("update step is: ", update_step)

        if update_step==0 or (update_step+1)%(40000)==0:
            video_recorder = None
            if alg.double_q:
                video_path = 'results/' + env_name + '_doubleQ'+'_itr' + str(update_step) +'.mp4'
            else:
                video_path = 'results/' + env_name +'_itr' + str(update_step) +'.mp4'
            video_recorder = VideoRecorder(alg.env, video_path, enabled=video_path is not None)

        alg.step_env(update_step,video_recorder)
