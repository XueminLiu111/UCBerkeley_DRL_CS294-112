...
def plot_42(env_name):
    ...
    bc1 = load_json(os.path.join(
        ...
        env_name + '_hidden200', 
        'behavioral_cloning', 
        env_name + '_hidden200_results.json'))
    ...
    plt.savefig(
        #os.path.join(EXPERIMENTS_DIR, env_name + '_smooth-l1', env_name + '.png'),
        os.path.join(EXPERIMENTS_DIR, env_name + '_hidden200', env_name + '.png'),
        bbox_inches='tight',
        transparent=True,
        pad_inches=0.1
    )
    #return os.path.join(EXPERIMENTS_DIR, env_name + '_smooth-l1', env_name + '.png')
    return os.path.join(EXPERIMENTS_DIR, env_name + '_hidden200', env_name + '.png')


def get_plots_23(env_list):
    with open('report.md', 'a') as f:
        f.write('### Question 2.3 behavioral cloning\n\n<p float="left">\n')
        for env_name in env_list:
...
def get_plots_41(env_list):
    with open('report.md', 'a') as f:
        f.write('### Question 4.1 change loss function of policy network\n\n<p float="left">\n')
        for env_name in env_list:
...
def get_plots_42(env_list):
    with open('report.md', 'a') as f:
        f.write('### Question 4.2 change hidden layer size of policy network\n\n<p float="left">\n')
        for env_name in env_list:
            path = plot_42(env_name)
            f.write('  <img src="{}" width="350"/>\n'.format(path))
        f.write('</p>\n\n')
