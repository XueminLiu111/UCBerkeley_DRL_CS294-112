...
class Model:
    ...
    def build_graph(self):  #3fc layers
        ...
        for i in range(1, len(self.FLAGS['hidden_dims'])):
            ...
            #=======xliu24 test
            #print("=====xliu24 hidden_dims=",self.FLAGS['hidden_dims'][i],"when i=",i)
            #import pdb
            #pdb.set_trace()
    ...
    def evaluate(self, session, env, num_rollouts, max_steps, epoch, video_path, video_recorder):
        returns = []
        observations = []
        ...
        for i in range(num_rollouts):
            ...
            if i==0 and epoch%20==0:
                ...
                rews_path = video_path[:-4] + '_total_rewards.npy' 
                pickle.dump(total, open(rews_path,"wb"))
                #pdb.set_trace()

        return returns, observations
    
    
    def update_expert_data(self, observations, data_train, data_val, epoch):
        ...
        #pdb.set_trace()
        #print('==== out put of expert_policy_fn is:', self.expert_policy_fn(observations))
        #print('==== epoch=',epoch)
        if epoch==5:
            env_expert = gym.make(self.FLAGS['env_name'])
            max_steps = self.FLAGS['max_timesteps'] or env_expert.spec.timestep_limit
            env_name_expert = env_expert.unwrapped.spec.id
            obs_expert = env_expert.reset()
            env_expert.render()
            done_expert = False
            total_expert = steps_expert = 0
            #video_recorder_expert = None
            video_path_expert = 'experiments/movies/' + env_name_expert + '_expert' +'.mp4'
            video_recorder_expert = VideoRecorder(env_expert, video_path_expert, enabled=video_path_expert is not None)
             

            while not done_expert:
                video_recorder_expert.capture_frame()
                #pdb.set_trace()
                obs_expert = np.expand_dims(obs_expert, axis=0)
                #obs_expert = obs_expert.T
                act_expert = self.expert_policy_fn(obs_expert)
                #pdb.set_trace()
                obs_expert, r_expert, done_expert, _ = env_expert.step(act_expert)
                print('=====expert move one step forward')
                total_expert += r_expert
                steps_expert += 1

                if steps_expert >= max_steps:
                    break

            print("Saved expert video.")
            video_recorder_expert.close()
            video_recorder_expert.enabled = False
            rews_path_expert = video_path_expert[:-4] + '_total_rewards.npy' 
            pickle.dump(total_expert, open(rews_path_expert,"wb")) 
            pdb.set_trace()

        #output of expert_policy_fn is action value which is a array with 6 entry
        return merge_data(data_train, d1), merge_data(data_val, d2)

    '''
    def record_expert(self): # record the performance of the expert
        ...
        total_expert = steps_expert = 0
        #video_recorder_expert = None
        video_path_expert = 'experiments/'+ env_name_expert +'/' + env_name_expert + '_expert' +'.mp4'
        video_recorder_expert = VideoRecorder(env_expert, video_path_expert, enabled=video_path_expert is not None)
        video_recorder_expert.capture_frame() 
        ...
        while not done_expert:
            ...
            obs_expert, r_expert, done_expert, _ = env.step(act_expert)
            total_expert += r_expert
            steps_expert += 1
    ...
    def train(self, session, curr_dir, bestmodel_dir, data_train, data_val):
        ...
        #record the initial vedio without training
        video_recorder_0 = None
        if self.algorithm == 'dagger':
            video_path_0 = 'experiments/movies/' + env_name + '_dagger' +'_h'+str(self.FLAGS['hidden_dims'][0]) +'_epc' + str(epoch) +'.mp4'
        else:
            video_path_0 = 'experiments/movies/' + env_name + '_bc' +'_h'+str(self.FLAGS['hidden_dims'][0]) +'_epc' + str(epoch) +'.mp4'
        video_recorder_0 = VideoRecorder(env, video_path_0, enabled=video_path_0 is not None)
        curr_returns_0, curr_observations_0 = self.evaluate(session, env, num_rollouts, max_steps, epoch, video_path_0, video_recorder_0)
        #pdb.set_trace()#                                            evaluate(self, session, env, num_rollouts, max_steps, epoch, video_path, video_recorder)
        ...
        while self.FLAGS['num_epochs'] == 0 or epoch < self.FLAGS['num_epochs']:
            ...
            #curr_returns, curr_observations = self.evaluate(session, env, num_rollouts, max_steps, epoch)
            video_path_0 = None
            video_recorder_0 = None
            if epoch%20==0:
                if self.algorithm == 'dagger':
                ...
                video_recorder_0 = VideoRecorder(env, video_path_0, enabled=video_path_0 is not None)
                #curr_returns_0, curr_observations_0 = self.evaluate(session, env, num_rollouts, max_steps, epoch, video_path_0, video_recorder_0)
            curr_returns, curr_observations = self.evaluate(session, env, num_rollouts, max_steps, epoch, video_path_0, video_recorder_0)
            ...
            if self.algorithm == 'dagger':
                ...
                data_train, data_val = self.update_expert_data(np.array(curr_observations), data_train, data_val, epoch)
