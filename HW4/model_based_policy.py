...
class ModelBasedPolicy(object):
    ...
    def _setup_action_selection(self, state_ph):
        ...
        states = tf.stack([state_ph[0]] * self._num_random_action_selection)  #stack 4096 current states
        for t in range(self._horizon):
            next_states = self._dynamics_func(states, actions[:, t, :], True) # predict next state by _dynamics_func, 
                                                                              # input is same current state, but different sampled 4096 actions
                                                                              # output is different 4096 next state
            costs += self._cost_fn(states, actions[:, t, :], next_states)
                                                                            # cost is sum of (1) and (2), where
                                                                            # (1) tendency to fall down due to tilt forward too much 
                                                                            # (2) difference of 17th dimension difference of predicted and ground truth
                                                                            # in summary, cost is penality of falling down and not moving faster 
            states = next_states
        best_action = actions[tf.argmin(costs)][0]  # choose best action sequence which minimize the cost among 4096 costs
                                                    # choose the first action in this action sequence
                                                    # Q: why does this action give us a large return?
                                                    # A: cost is penality of falling down and not moving faster,
                                                    #    which intend to increase the reward, therefore return. 
        return best_action
