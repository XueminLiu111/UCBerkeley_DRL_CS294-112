...
class ModelBasedRL(object):
    ...
    def __init__(self,
                 ...
                 max_rollout_length=500,  #100, #for test  #500,
        ...
        #===xliu24==== add for movie recording
    ...
    def _gather_rollouts(self, policy, num_rollouts):
        ...
        for _ in range(num_rollouts):
            ...
            while not done:
                ...
                action = policy.get_action(state)   # select best action (in 4096)which minimize the cost function of dynamics
                                                    # cost is penality of falling down and not moving faster 
                                                    # in each rollout, we select num_random_action_selection=4096 sequences of action
                                                    # eqch sequence contain horizon=15 actions
                                                    # but we only act the first action to move to next state
    ...
    #===xliu24==== define a function to record movie
    def record_movie(self, policy, itr):
        env_recode = self._env
        #env_recode_name = env_recode.unwrapped.spec.id
        state = env_recode.reset()
        done = False
        t = 0
        env_recode.render()
        video_recorder = None
        video_path = 'data/'+ 'HalfCheetah_q3' + '_horizon'+str(self._mpc_horizon) \
                        + '_action' +str(self._num_random_action_selection) \
                        + '_layers' +str(self._nn_layers) \
                        + '_itr' + str(itr) +'.mp4'        
        video_recorder = VideoRecorder(env_recode, video_path, enabled=video_path is not None)
        while not done:
            ...
            action = policy.get_action(state)   # select best action (in 4096)which minimize the cost function of dynamics
                                                    ...
                                                    # eqch sequence contain horizon=15 actions
                                                    # but we only act the first action to move to next state
            next_state, reward, done, _ = env_recode.step(action)
            done = done or (t >= self._max_rollout_length)
            state = next_state
            t += 1
        print("Saved video.")
    ...
    def _train_policy(self, dataset):
        ...
        for i in range(self._training_epochs):
            for states, actions, next_states, rewards, dones in dataset.random_iterator(self._training_batch_size):
                ...
                # loss is difference between predicted next state and ground truth
    ...
    def run_q1(self):
        ...
        ### PROBLEM 1
        ### YOUR CODE HERE
        self._train_policy(self._random_dataset)  # train the dynamic function using random dataset
                                                  # Q: what is the size of dataset?
                                                  # A: 10 num_init_random_rollouts, 500 max_rollout_length

        logger.info('Evaluating predictions...')  # evaluate the dynamic function after training (dataset is same as training)
        for r_num, (states, actions, _, _, _) in enumerate(self._random_dataset.rollout_iterator()):
            ...
            pred_state = states[0]
            for act in actions:
                pred_state = self._policy.predict(pred_state, act)  #_policy here is the dynamic model
            ...
            rows = int(np.sqrt(state_dim))  # plot figure for each dimension, in the HalfCheetah, state is 20 dimemsion
            cols = state_dim // rows
    ...
    def run_q2(self):
        ...
        ### PROBLEM 2
        ### YOUR CODE HERE
        # gather evaluation dataset which is random but not same with training dataset
        # _num_onpolicy_rollouts = 10, so evaluation dataset has same size with training dataset
    ...
    def run_q3(self):
        ...
        self._log(dataset)
        '''
        video_path_0 = 'data/'+ '_horizon'+str(self._mpc_horizon) \
                        + '_action' +str(self._num_random_action_selection) \
                        + '_layers' +str(self._nn_layers) \
                        + '_itr' + str(itr) +'.mp4'
        '''                
        self.record_movie(self._policy, itr)
        #pdb.set_trace()
        ...
        for itr in range(self._num_onpolicy_iters + 1):
            ...
            ### YOUR CODE HERE
            logger.info('Training policy...')
            self._train_policy(dataset)  # the dataset used to train is updated in each iteration by implementing the first action

            ### PROBLEM 3
            ### YOUR CODE HERE
            logger.info('Gathering rollouts...')  # same as q2 evaluating policy
            ...
            self._log(new_dataset)  # evaluating the policy on new dataset
