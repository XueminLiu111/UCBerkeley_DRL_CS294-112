...
class PointMass(Env):
    def __init__(self, max_episode_steps_coeff=1, scale=20, goal_padding=2.0):
        ...
        # state sapce is [0,1]^2
        self.observation_space = gym.spaces.Box(
            low=np.array([0.0, 0.0]),
            high=np.array([1.0, 1.0]))
        # action sapce is [-inf,+inf]^2
    ...
    def reset(self):
        plt.close()
        self.state = np.array([self.goal_padding, self.goal_padding])  # (2,2)
        state = self.state/self.scale  # inital state (0.1, 0.1)
        return state

    def step(self, action):
        x, y = action   # action is move in x and y direction
        ...
        threshold = self.scale - self.goal_padding  # threshold = 20-2 = 18
        if new_x > threshold and new_y > threshold:
            reward = 10 + reg_term  # reward for move the point outside the square (0.9,0.9)
        else:
            reward = 0 + reg_term  # reward for move the point inside the square (0.9,0.9)
    ...
    def preprocess(self, state):
        ...
        x_floor, y_floor = np.floor(scaled_state)  #The floor of the scalar x is the largest integer i, such that i <= x. It is often denoted as \lfloor x \rfloor.
        assert x_floor <= self.scale
        assert y_floor <= self.scale
        if x_floor == self.scale:
            x_floor -= 1  # max floor is 19
    ...
    def unprocess(self, index):
        x_floor = index // self.scale  # // is floor division
        y_floor = index % self.scale   # why not index//self.scale ??
    ...
    def render(self):
        ...
        for i in indices:
            a[i] += 1
        max_freq = np.max(a)  # find the most frequent index(position)
        a/=float(max_freq)  # normalize
        a = np.reshape(a, (self.scale, self.scale))
        ax = sns.heatmap(a)  # Plot rectangular data as a color-encoded matrix.
                             # the color is the normalized times of that position
        plt.draw()
        plt.pause(0.001)  #Run the GUI event loop for interval seconds.
        plt.clf()        
