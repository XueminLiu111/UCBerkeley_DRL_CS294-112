...
class ExemplarExploration(ContinuousExploration):
    ...
    def sample_idxs_replay(self, states, batch_size):
        ...
        positives = np.concatenate([hid_dim[pos_idxs], states[pos_idxs]], axis=0)
    ...
    def fit_density_model(self, states):
        """
            args:
                states: (bsize, ob_dim)
                # bsize is number of samples to train the discri
        """
        self.replay_buffer.prepend(states)
        for i in range(self.train_iters):
            ...
            ll, kl, elbo = self.density_model.update(positives, negatives, labels)  # train the discriminator
