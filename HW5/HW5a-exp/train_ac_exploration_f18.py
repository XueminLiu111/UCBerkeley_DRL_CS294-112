...
#========= xliu24 ============
# define a function to record the video
def record_animation(agent, env, video_path):
    max_step = 150  # selected for HalfCheetah-v2 env
    video_recorder = None
    ...
    while step<=max_step and not done:
        ...
        #pdb.set_trace()
        action = agent.sess.run(agent.sy_sampled_ac, feed_dict={agent.sy_ob_no: obs.reshape(1, -1)})
        action = action[0]
        #pdb.set_trace()
...
'''
#no need. we can generate fig file after trainig by run: python pointmass.py <dirname>
def record_animation_PointMass(agent, env, itr, logdir):
	#logdir = 'pm_movie'
    #os.mkdir(logdir)
    max_step = 20  # find in pointmass.py
    states = []
    state = env.reset()
    done = False
    episode_rew = 0
    step = 0
    while step<=max_step and not done:
    	action = agent.sess.run(agent.sy_sampled_ac, feed_dict={agent.sy_ob_no: state.reshape(1, -1)})
    	action = action[0]
    	state, reward, done, _ = env.step(action)
    	states.append(state)
    	episode_rew += reward
    	step += 1
    env.visualize(np.array(states), itr, logdir)
    print("Episode reward", episode_rew)
    print("Saved video.")
    return episode_rew
'''
...
def train_AC(
    ...
    # record the model performance without training

    if env_name == 'PointMass-v0':
        video_path= 'results/' + env_name + '_dm'+ dm +'_' + '_bc'+ str(bonus_coeff) \
                        + '_seed' + str(seed) +'_b' + str(min_timesteps_per_batch)+ '_lr' + str(learning_rate) +'_itr'
        os.mkdir(video_path)
        episode_rews = [] 
        episode_rew =record_animation_PointMass(agent, env, 0, video_path)
        episode_rews.append(episode_rew)



    if env_name == 'HalfCheetah-v2':
        video_path_init = 'results/' + env_name + '_dm'+ dm +'_' + '_bc'+ str(bonus_coeff) \
                        + '_seed' + str(seed) +'_b' + str(min_timesteps_per_batch)+ '_lr' + str(learning_rate) +'_itr0' + '.mp4'
        episode_rews = []
        episode_rew = record_animation(agent, env, video_path_init)
        episode_rews.append(episode_rew)
        #pdb.set_trace()
    ...
    for itr in range(n_iter):
        ...
        if env_name == 'PointMass-v0':
            if (itr+1)%20==0:
                episode_rew = record_animation_PointMass(agent, env, itr, video_path)
                episode_rews.append(episode_rew)

        if env_name == 'HalfCheetah-v2':
            ## test and record vedios every 20 iteration
            if (itr+1)%20==0:
                #video_path = 'results/' + env_name + '_e' + str(num_e) +'_itr' + str(itr) + '.mp4'
                #video_path_init = 'results/' + env_name + '_' +str(num_target_updates) +'_' + str(num_grad_steps_per_target_update)\
                #        + '_e' + str(num_e) +'_b' + str(min_timesteps_per_batch)+ '_lr' + str(learning_rate) +'_itr0' + '.mp4'
                video_path = video_path_init[:-5] + str(itr) + '.mp4'
                #video_path ='results/' + env_name + '_' +str(num_target_updates) +'_' + str(num_grad_steps_per_target_update)\
                #        + '_e' + str(num_e) +'_b' + str(min_timesteps_per_batch) + '_lr' + str(learning_rate) + '_itr' +str(itr) + '.mp4'
                episode_rew = record_animation(agent, env, video_path)
                episode_rews.append(episode_rew)

    if env_name == 'PointMass-v0':
        env.create_gif(video_path)
        print(episode_rews)
        video_path = video_path[:-4] + '_test_rewards.npy'
        #rews_path = 'results/' + env_name +  '_' +str(num_target_updates) +'_' + str(num_grad_steps_per_target_update)\
        #            + '_e' + str(num_e) +'_b' + str(min_timesteps_per_batch) + '_lr' + str(learning_rate) + '_test_rewards.npy'
        pickle.dump(episode_rews, open(video_path,"wb"))
        

    if env_name == 'HalfCheetah-v2':
        print(episode_rews)
        rews_path = video_path_init[:-9] + '_test_rewards.npy'
        #rews_path = 'results/' + env_name +  '_' +str(num_target_updates) +'_' + str(num_grad_steps_per_target_update)\
        #            + '_e' + str(num_e) +'_b' + str(min_timesteps_per_batch) + '_lr' + str(learning_rate) + '_test_rewards.npy'
        pickle.dump(episode_rews, open(rews_path,"wb"))



#load rews data in .npy file
def load_rews(rews_path):
    rewards = np.load(rews_path, allow_pickle=True)
...
def main():
    ...
    parser.add_argument('--kl_weight', '-kl', type=float, default=1e-2) #ex2
    parser.add_argument('--density_lr', '-dlr', type=float, default=5e-3) #ex2
    parser.add_argument('--density_train_iters', '-dti', type=int, default=1000) #ex2: iteration for train density model
    parser.add_argument('--density_batch_size', '-db', type=int, default=64) #ex2: number of sates in ex2
    parser.add_argument('--density_hiddim', '-dh', type=int, default=32) #ex2: hidden layer size for discriminator network and encorder network
                                                                         #ex2: the half is output size for encorder network, which is size of latent variables
    parser.add_argument('--replay_size', '-rs', type=int, default=int(1e6))  #rbf:number of total sates to calculate f_{\phi}(s)
    parser.add_argument('--sigma', '-sig', type=float, default=0.2)  #rbf:sigma for Guassian
    ########################################################################
