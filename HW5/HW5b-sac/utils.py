...
class SimpleSampler(Sampler):
    ...
    def sample_record(self, policy=None):
        env_name = self.env.unwrapped.spec.id
        video_path = 'results/' + env_name + '_reparam' +'_epc'  + str(self._epoch) + '.mp4'
        video_recorder = None
        video_recorder = VideoRecorder(
            self.env, video_path, enabled=video_path is not None)
        policy = self.policy if policy is None else policy
        #if self._current_observation is None:
        max_episode_length = self._max_episode_length
        obs, done, episode_length, episode_return = self.env.reset(), False, 0, 0
