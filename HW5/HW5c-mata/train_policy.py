...
from point_mass import PointEnv2, PointEnv3
...
def build_policy(x, h, output_size, scope, n_layers, size, gru_size, recurrent=True, activation=tf.tanh, output_activation=None):
    ...
    with tf.variable_scope(scope, reuse=tf.AUTO_REUSE):
        ...
        x = tf.layers.dense(x, output_size, activation=output_activation, \
            kernel_initializer=tf.initializers.truncated_normal(mean=0.0, stddev=0.01), bias_initializer=tf.zeros_initializer(), name='decoder')
    return x, h
...
class Agent(object):
    ...
    def sample_trajectory(self, env, animate_this_episode, is_evaluation):
        ...
        # if 'pm-obs' env, already identify the goal in the env
        # if 'pm3', the goal is random, not fixed 4 goals
        ...
        while True:
            ...
            # YOUR CODE HERE
            #input length to the gru is elf.history 
            ...
            # YOUR CODE HERE
            meta_obs[self.history + ep_steps - 1] = np.concatenate((next_ob, ac, [rew], [done]))  # new meta_obs
                                                                                                  # add one for each env.step
            ...
            if done:
                ...
                s['ep_len'] = ep_steps
                stats.append(s)  # only record rewards and episode length
                ep_steps = 0
    ...
    def compute_advantage(self, ob_no, re_n, hidden, masks, tau=0.95):
        ...
        arguments:
            ...
            values: (bsize,)
            gamma: scalar
            tau: scalar  # is this lamda in hw2?
        ...
        #print('------ re_n shape is ---', re_n.shape)
        ...
        prev_value = 0
        prev_advantage = 0
        for i in reversed(range(bsize)):  # reverse order in bsize, for last one to first one
            #b = rewards[i] + gamma * prev_return * masks[i]
            #print('----- right side is ---', b)
            #print('----- left side is---', returns[i])
...
def train_PG(
    ...
    for itr in range(n_iter):
        ...
        stats = []
        for _ in range(num_tasks): # randomly sample trjectories from 4 tasks 
        ...
        for _ in range(num_ppo_updates):
            ...
            #why comput log_probs given fixed_log_probs
        ...
        print('Validating...')
        val_stats = []
        for _ in range(num_tasks):  # why loop for 4 tasks?  #only validate on 1st goal (-10,-10)???
