...
class ObservedPointEnv(Env):
    ...
    def reset_task(self, is_evaluation=False, grain_size=None):
        ...
        if is_evaluation: # evaluation on task0
            self.task_idx = (self.task_idx + 1) % len(self.tasks)
        # during training, sample tasks randomly
        else:
            self.task_idx = np.random.randint(len(self.tasks)) #Return random integers in self.tasks
        self._task = self.tasks[self.task_idx]
        goals = [[-1, -1], [-1, 1], [1, -1], [1, 1]]
        self._goal = np.array(goals[self.task_idx])*10

    def reset(self):
        self._state = np.array([0, 0], dtype=np.float32)  #starting form (0,0)
